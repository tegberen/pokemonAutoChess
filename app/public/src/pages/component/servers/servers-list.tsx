          ...
          (Object.entries(config) as [string, ServerInfo][]).map(
            ([name, server]) => {
              return {
                ...
                logo:
                  server.logo ??
                  "https://pokemon-auto-chess.com/assets/ui/pokemon_autochess_final.svg",
...
export function ServerInfo(props: { server: ServerInfo }) {
  ...
  return (
    <li className={cc("server my-box", { current: isCurrentServer })}>
      <header>
        ...
        {!isCurrentServer && (
          <a className="bubbly blue" href={server.url}>
            {t("switch_server")}
          </a>
        )}
      ...
      <dl>
        ...
        <dd className={"status-" + serverStatus.status}>
          {serverStatus.status}
        </dd>
        {serverStatus.status === "online" && (
          <>
            ...
            <dt>{t("players")}</dt>
            <dd>
              {serverStatus.ccu}/{serverStatus.maxCcu}
            </dd>
            <dt>{t("ping")}</dt>
