...
export default abstract class PokemonState {
  ...
  attack(
    ...
    if (target && target.hp > 0) {
      ...
      if (specialDamage > 0) {
        ...
        const { takenDamage, death } = target.handleSpecialDamage(
          specialDamage,
          board,
          AttackType.SPECIAL,
          pokemon,
  ...
  triggerDeath(
    ...
    pokemon.simulation.onStoneSaddleMountFallen(pokemon)
    ...
    if (pokemon.simulation.weather === Weather.SANDSTORM) {
      ...
      if (pokemon.sandstormDamageTimer <= 0 && !pokemon.simulation.finished) {
        ...
        if (
          ...
          // Sandstorm is board-wide weather with no caster; credit its chip to
          // the enemy team's Sandstorm row (see creditSyntheticDamage).
